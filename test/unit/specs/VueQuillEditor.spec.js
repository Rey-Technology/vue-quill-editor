...
describe('rey-vue-quill-editor', () => {
  ...
  describe('Component data binding', () => {
    it(' - should change the quill content after change the component content data', (done) => {
      ...
      Vue.nextTick(() => {
        ...
        expect(vm.quill.editor.delta.ops).to.deep.equal([
          { insert: 'test change\n' },
        ]);
        done();
      });
  ...
  describe('Multi edirot component instance', () => {
    it(' - should update value after any change text', (done) => {
      ...
      expect(vm.$refs.editora[0].quill.getText()).to.deep.equal(
        'a-test content\n'
      );
      expect(vm.$refs.editorb[0].quill.getText()).to.deep.equal(
        'b-test content\n'
      );
      expect(vm.$refs.editorc[0].quill.getText()).to.deep.equal(
        'c-test content\n'
      );
